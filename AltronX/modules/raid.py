...
async def spam(e):
    ...
    if e.sender_id in SUDO_USERS:
        ...
        if len(mkraid) == 2:
            ...
            g = a.id
            if int(g) in ALTRON:
                await e.reply("» αвє ᴊнαтυ вσ ѕαᴄнιи ѕαиαтαиι нαι мтℓв тєʀα вααρ", parse_mode=None, link_preview=None)
            elif int(g) in SUDO_USERS:
                await e.reply("» αвє ує тσ ѕαиαтαиι кα ѕυ∂σ υѕєʀ нαι вє", parse_mode=None, link_preview=None)
            else:
                c = a.first_name
    ...
    if e.sender_id in SUDO_USERS:
        ...
        if len(e.text) > 11:
            ...
            if int(user_id) in ALTRON:
                ...
                await e.reply("» αвє ує тσ тєʀα вααρ нαι ᴊнαтυ", parse_mode=None, link_preview=None)
                ...
                phucker = [user_id]
                gey.append(phucker)
                await e.reply("» αвє αв ʀαℓρу кαʀ кαʀиє ρє gαℓιуα ∂υиgα вσ внι мαѕт мαѕт", parse_mode=None, link_preview=None)
            ...
            if int(user_id) in ALTRON:
                ...
                phucker = [user_id]
                gey.append(phucker)
                await e.reply("» ᴄнαℓ ᴊα ᴄннσ∂ ∂ιуα αв ʀαρℓу кαʀиє ρє gαι иαнι ∂υиgα", parse_mode=None, link_preview=None )
        else:
            await e.reply(usage)
